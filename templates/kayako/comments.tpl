<div class="commentslabel">Comments ({$_commentCount})</div>
	...
	<div style="padding-left: {$_comment.padding}px;">
		...
		<div style="">{if $_canPostComments != false}<a href="javascript: void(0);" onclick="javascript: MoveCommentReply('{$_commentID}');">{$LANG.integrakayakoreply}</a>{/if}</div>
		</div>
	...
	<div id="commentparent">
		...
		<table class="hlineheader"><tr><th rowspan="2" nowrap><div id="postnewcomment">{$LANG.integrakayakoposta}</div><div id="replytocomment" style="display:none;" >{$LANG.integrakayakoreplyto}</div></th><td>&nbsp;</td></tr><tr><td class="hlinelower">&nbsp;</td></tr></table>
		<table width="100%" border="0" cellspacing="1" cellpadding="4">
			{if $_commentsFullName == ''}
			<tr>
				<td width="200" align="left" valign="middle" class="zebraodd">{$LANG.integrakayakofullname}</td>
				<td><input name="fullname" type="text" size="25" class="swifttextlarge" value="{$_commentsPostFullName}" /></td>
			</tr>
			{/if}
			{if $_commentsEmail == ''}
			<tr>
				<td align="left" valign="middle" class="zebraodd">{$LANG.integrakayakoemail}</td>
				<td><input name="email" type="text" size="25" class="swifttextlarge" value="{$_commentsPostEmail}" /></td>
			</tr>
			{/if}
			<tr>
				<td align="left" valign="top" class="zebraodd">{$LANG.integrakayakocommentss}</td>
